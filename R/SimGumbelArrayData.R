...
write.csv(seed_test, paste0(seed_dir, '/', job_id, '_', task_id, '_seed_test.csv'), row.names = F)

# Simulate the data
data <- SimData(n = observations, pars = pars, trans_mat = trans_mat, family = family)
...
write.table(data$U, paste0(data_dir, '/', job_id, '_', task_id, '_data.csv'), row.names = F, col.names = F, sep = ',')
write.table(data$states, paste0(data_dir, '/', job_id, '_', task_id, '_states.csv'), row.names = F, col.names = F, sep = ',')
write.table(data$theta, paste0(data_dir, '/', job_id, '_', task_id, '_theta.csv'), row.names = F, col.names = F, sep = ',')
write.table(data$theta_sim, paste0(data_dir, '/', job_id, '_', task_id, '_theta_sim.csv'), row.names = F, col.names = F, sep = ',')
