#' pClaytonCopula
#'
#' A function to compute the distribution function of the Clayton copula.
#'
#' This function computes the Clayton copula distribution function at \eqn{n}
...
#' the cdf of the Clayton copula.
#' @param theta scalar dependence parameter for the Clayton copula. Restricted
#' to the interval \eqn{[0, \infty)}.
#'
...
#' cdfClayton <- pClaytonCopula(U, theta)
pClaytonCopula <- function(U, theta) {
  ...
  if (any(U < 0) | any(U > 1)) {
    stop('U contains values outside the interval [0, 1]')
  }
...
#' dClaytonCopula
#'
#' A function to compute the Clayton copula density.
#'
#' This function computes the Clayton copula density function at \eqn{n} points,
...
#' U <- matrix(runif(n * d), ncol = d)
#' pdfClayton <- dClaytonCopula(U, theta)
dClaytonCopula <- function(U, theta) {
  # Check if U is a matrix and coerce to matrix form if necessary. We assume that
  # U is a (1 x d) vector in this instance.
  if (!any(class(U) %in% c('matrix'))) {
    U <- matrix(U, nrow = 1)
  ...
  pdfexpr <- ClaytonPDFexpr(d)

  pdf_eval_fun <- function(i, U, theta, dim) {
    return(ClaytonPDFeval(U[i, ], dim , theta[i]))
  }
...
#' ClaytonLogLik
#'
#' A function to compute the Clayton copula log-likelihood.
#'
#' This function computes the Clayton copula log-likelihood at \eqn{n} points,
#' where the \eqn{i}th point in \eqn{d} dimensions is specified by the \eqn{i}th
#' row of the matrix \code{U}. The copula dependence parameter used to compute
...
#' log-likelihood of the Clayton copula.
#'
...
#' theta <- runif(n, min = 0, max = 10)
#' U <- matrix(runif(n * d), ncol = d)
#' loglikClayton <- ClaytonLogLik(theta, U)
ClaytonLogLik <- function(theta, U) {
  # Check if U is a matrix and coerce to matrix form if necessary. We assume that
  # U is a (1 x d) vector in this instance.
  if (!any(class(U) %in% c('matrix'))) {
    U <- matrix(U, nrow = 1)
  ...
  loglik <- log(dClaytonCopula(U, theta))

  loglik[is.infinite(loglik)] <- -1e9
  return(loglik)
}

# A helper function used to generate an expression for the cdf of the Clayton
# copula for a given number of dimensions
ClaytonCDFexpr <- function(dim) {
  CDFexpr <- paste0('(u', 1:dim, '^(-theta) - 1)', collapse = ' + ')
  CDFexpr <- paste0('(', CDFexpr, ' + 1)^(-1/theta)')
  CDFexpr <- parse(text = CDFexpr)
  return(CDFexpr)
}

# A helper function used to generate an expression for the pdf of the Clayton
# copula for a given dimension
ClaytonPDFexpr <- function(dim) {
  CDFexpr <- ClaytonCDFexpr(dim)
  PDFexpr <- CDFexpr
  for (i in 1:dim) {
...
# A helper function used to evaluate the expression for the pdf of the Clayton
# copula for a given dimension
ClaytonPDFeval <- function(u, dim, theta) {
  pdfexpr <- ClaytonPDFexpr(dim)
  for (i in 1:dim) {
