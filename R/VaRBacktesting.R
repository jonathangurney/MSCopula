...
switch(family, 
       "clayton" = {
         psi <- mean(abs(U[(observations - 9):observations, 1] -
                           U[(observations - 9):observations, 2]))
       ...
       "gaussian" = {
         psi <- mean(qnorm(U[(observations - 9):observations, 1]) *
                       qnorm(U[(observations - 9):observations, 2]))
...
write.table(HF_copula_GOF,
            HF_copula_GOF_filename, sep = ',', row.names = F, col.names = F)
write.table(EM_copula_GOF,
            EM_copula_GOF_filename, sep = ',', row.names = F, col.names = F)
